...
function productDetailHref(productId) {
  return `product-${String(productId).replace('prod-', '')}.html`;
}
    ...
    return `
      <article class="card-editorial reveal-on-scroll is-visible" data-product-id="${product.id}">
        ...
        <h3 style="font-size: 1.35rem; margin-bottom: 0.6rem; min-height: 2.8rem; line-height: 1.2;">
          <a href="${productDetailHref(product.id)}">${product.name}</a>
        </h3>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; align-items: start;">
      ...
      <div>
        ...
        <div style="display: flex; gap: 0.75rem;">
          ...
          <a href="${productDetailHref(product.id)}" class="btn btn-outline-burgundy">Full View</a>
        </div>
      </div>
    </div>
  `;
